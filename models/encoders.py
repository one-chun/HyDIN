    ...
    def __init__(self, c, args):
        ...
        if args.pretrained_embeddings is not None and args.num_layers > 0:
            ...
            for i in range(len(dims) - 1):
                in_dim, out_dim = dims[i], dims[i + 1]
                act = acts[i]
                layers.append(hyp_layers.LorentzLinear(self.manifold, in_dim, out_dim, args.bias, args.dropout, 10,
                                                       nonlin=act if i != 0 else None))
        self.layers = nn.Sequential(*layers)
        self.encode_graph = False
        if args.use_feats:
            self.transform = hyp_layers.LorentzLinear(self.manifold, args.feat_dim + 1, args.dim, args.bias,
                                                      args.dropout, 10)
